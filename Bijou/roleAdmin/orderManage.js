...
function formatRevenue(totalRevenue) {
  if (totalRevenue > 1_000_000_000)
    return (totalRevenue / 1_000_000_000).toFixed(2) + "B";
  if (totalRevenue > 1_000_000)
    return (totalRevenue / 1_000_000).toFixed(2) + "M";
...
async function fetchGraph(year) {
  try {
    ...
    const totalRevenue = data.reduce(
      (sum, item) => sum + item.currentYearRevenue,
      0
    );
    ...
    document.querySelector(".total-amount-order").innerText =
      formatRevenue(totalRevenue);
    document.querySelector(".sale-qty").innerText = sales;

    if (!groupChartEarning) {
      groupChartEarning = new ApexCharts(
...
async function ShowInvoices(start, end) {
  try {
    ...
    invoiceTableBody.innerHTML = topInvoices
      .map(
        (invoice) => `
      <tr data-id="${invoice.ID}" class="InvoiceLine">
        ...
        <td><span class="status ${invoice.Status.toLowerCase().replace(
          /\s+/g,
          ""
        )} editStatus">${invoice.Status}</span></td>
      </tr>
    `
      )
      .join("");
...
async function showDetailForm(invoice) {
  try {
    ...
    // Set the title
    document.getElementById("popupTitleOrderDetails").innerText =
      "View Order Details";
      ...
      .flatMap((item) =>
        ...
        <tr>
          ...
          <td><span class="status ${item.Status.toLowerCase().replace(
            /\s+/g,
            ""
          )} editStatus">${item.Status}</span></td>
        </tr>`;
          i++;
          return row;
        })
    ...
    document.getElementById("totalAmount").innerText = `${formatPrice(
      totalAmount
    )} VND`;
...
async function ShowRecentCustomers(number = null) {
  try {
    ...
    customerTableBody.innerHTML = topCustomers
      .map(
        (customer) => `
      <tr>
        <td width="60px">
...
function toggleEditMode() {
  ...
  localStorage.setItem(
    "editMode",
    table.classList.contains("edit-mode") ? "on" : "off"
  );
  this.textContent = table.classList.contains("edit-mode")
    ? "Save"
    : "Edit Mode";
}
  ...
  try {
    ...
    if (data) {
      ...
      invoiceTableBody.innerHTML = `
        <tr class="InvoiceLine" data-id="${data.ID}">
          <td>${data.ID}</td>
          <td>${data.customer.FirstName} ${data.customer.LastName}</td>
          <td>${formatRevenue(data.Price)}</td>
          <td>${data.Payment}</td>
          <td><span class="status ${data.Status.toLowerCase().replace(
            /\s+/g,
            ""
          )} editStatus">${data.Status}</span></td>
        </tr>
      `;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  prevButton.addEventListener("click", () => {
    ...
    if (start === 0) console.log("nomorepage");
    else {
      start -= 10;
      end -= 10;
